...
@property (nonatomic, copy) RCTDirectEventBlock onCameraCapture;
...
- (void)onCameraCapture:(NSDictionary *)event
{
    if (_onCameraCapture) {
        _onCameraCapture(event);
    }
}
...
- (void)startLiveness:(BOOL)isStart {
    if(isStart) {
        [self setupOrDisableLiveness];
        [self startSession];
    } else {
        [self stopSession];
    }
           ...
           fromConnection:(AVCaptureConnection *)connection
{
    ...
    BOOL canSubmitForTextDetection = timePassedSinceSubmittingForText > 0.2 && _finishedReadingText ;
