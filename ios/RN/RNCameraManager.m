...
RCT_EXPORT_VIEW_PROPERTY(onCameraCapture, RCTDirectEventBlock);
...
{
    return @[@"onCameraReady", @"onAudioInterrupted", @"onAudioConnected", @"onMountError", @"onBarCodeRead", @"onFacesDetected", @"onPictureTaken", @"onCameraCapture",@"onPictureSaved", @"onRecordingStart", @"onRecordingEnd", @"onTextRecognized", @"onGoogleVisionBarcodesDetected", @"onSubjectAreaChanged",@"onTouch"];
}
                 ...
                 rejecter:(RCTPromiseRejectBlock)reject)
{
    [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNCamera *> *viewRegistry) {
        ...
        if (![view isKindOfClass:[RNCamera class]]) {
            ...
            [view onPictureTaken:@{}];
//            [view onCameraCapture:@{@"data": @"onPictureTaken"}];
...
RCT_EXPORT_METHOD(startLiveness:(nonnull NSNumber *)reactTag)
{
    #if TARGET_IPHONE_SIMULATOR
        return;
    #endif
    [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNCamera *> *viewRegistry) {
        ...
        if (![view isKindOfClass:[RNCamera class]]) {
            ...
            [view startLiveness:YES];
        }
    }];
}

RCT_EXPORT_METHOD(stopLiveness:(nonnull NSNumber *)reactTag)
{
    #if TARGET_IPHONE_SIMULATOR
        return;
    #endif
